...
class UdpShowTransport:
    ...
    def send_payload(self, pins: Dict[str, Dict[str, int]], payload: Dict[str, Dict[int, Dict[str, dict]]]) -> None:
        ...
        try:
            ...
            sock.sendto(json.dumps(pins).encode("utf-8"), (self.config.host, self.config.port))
            sock.sendto("partiture".encode("utf-8"), (self.config.host, self.config.port))

            for slave_pin, slave_payload in payload.items():
                sock.sendto(slave_pin.encode("utf-8"), (self.config.host, self.config.port))
                sock.sendto(json.dumps(slave_payload).encode("utf-8"), (self.config.host, self.config.port))
