...
def apply_fill_range(
    colors: List[List[int]], start: int, end: int, rgb: List[int]
) -> List[List[int]]:
...
def moving_window_frames(
    led_count: int, window_size: int, rgb: List[int]
) -> List[List[List[int]]]:
    if led_count <= 0:
        return []
    if window_size <= 0:
...
def floating_gradient_frames(
    led_count: int, rgb: List[int], width: int = 4
) -> List[List[List[int]]]:
    if led_count <= 0:
        return []
    if width <= 0:
