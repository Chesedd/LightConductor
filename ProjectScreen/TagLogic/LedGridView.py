...
class LedGridView(QWidget):
    ...
    def _apply_sizing_from_slave(self, slave):
        """Update fixed height based on slave grid.
        Called from _recompute after _resolve_slave."""
        if slave is None:
            self._strip_mode = True
            self.setFixedHeight(STRIP_ROW_HEIGHT_PX)
            return
        ...
        try:
            ...
            if slave is not None:
                rows = max(
                    1,
                    int(
                        getattr(slave, "grid_rows", 1) or 1,
                    ),
                )
                cols = max(
                    1,
                    int(
                        getattr(slave, "grid_columns", 0) or 0,
                    ),
                )
            ...
            if self._strip_mode:
                # 1×N cells, stretched to fill widget
                # height (legacy visual).
