...
class LedPreviewOverlayTests(unittest.TestCase):
    def test_overlay_type_missing_returns_base(self):
        tt = make_tt(
            "alpha", "1", [0, 1],
            tags=[Tag(
                time_seconds=0.0, action=True,
                colors=[[255, 0, 0], [255, 0, 0]],
            )],
        )
        slave = make_slave(3, {"alpha": tt})
        result = render_led_strip_with_overlay(
            slave, 0.0, "beta", [[0, 255, 0]], True,
        )
        self.assertEqual(
            [(255, 0, 0), (255, 0, 0), (0, 0, 0)], result,
        )
    ...
    def test_overlay_replaces_existing_tag_of_same_type(self):
        ...
        result = render_led_strip_with_overlay(
            slave, 0.0, "alpha",
            [[0, 255, 0], [0, 255, 0], [0, 255, 0]], True,
        )
        self.assertEqual(
            [(0, 255, 0), (0, 255, 0), (0, 255, 0)], result,
        )
        self.assertIs(type(result[0]), tuple)
    ...
    def test_overlay_topology_oor_skipped(self):
        ...
        result = render_led_strip_with_overlay(
            slave, 0.0, "alpha",
            [[255, 0, 0], [255, 0, 0], [255, 0, 0]], True,
        )
        self.assertEqual(
            [(255, 0, 0), (0, 0, 0), (255, 0, 0)], result,
        )
    ...
    def test_overlay_on_nonexistent_time_base_is_off(self):
        tt = make_tt(
            "alpha", "1", [0],
            tags=[Tag(
                time_seconds=5.0, action=True,
                colors=[[255, 0, 0]],
            )],
        )
        slave = make_slave(1, {"alpha": tt})
        result = render_led_strip_with_overlay(
            slave, 0.0, "alpha", [[0, 255, 0]], True,
        )
    ...
    def test_overlay_string_action_on(self):
        tt = make_tt("alpha", "1", [0], tags=[])
        slave = make_slave(1, {"alpha": tt})
        on_result = render_led_strip_with_overlay(
            slave, 0.0, "alpha", [[255, 0, 0]], "On",
        )
        self.assertEqual([(255, 0, 0)], on_result)
        self.assertIs(type(on_result[0]), tuple)

        off_result = render_led_strip_with_overlay(
            slave, 0.0, "alpha", [[255, 0, 0]], "Off",
        )
